...
def main():
    ...
    while True:
        ...
        while wl_error == True:
            try:
                tidesDataFrame = tides.get_tide_data_for_plot()
                wl_error = False
        ...
        draw.text((660, 200), nx_nx_precip_percent, font=style.FONT_15, fill=style.BLACK)
        ...
        while hilo_error == True:
            try:
                extremeTideDF = tides.get_tide_extremes()
                hilo_error = False
            except:
                screen.display_error("Tide Prediction")

        # Display tide preditions
        y_loc = 300  # starting location of list
        # Iterate over preditions
        for index, row in extremeTideDF.iterrows():
            # For high tide
